...
const docSections = [
  ...
  },
  {
    title: 'User Guide',
    icon: Users,
    colorClass: 'bg-green-50 text-green-700 dark:bg-green-500/10 dark:text-green-400',
    links: [
...
export default function DocsPage() {
  ...
  return (
    <main className="min-h-screen transition-colors bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
            ...
            <div className="mx-auto mt-8 max-w-2xl">
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-10 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
                />
              </div>
            </div>
          </div>
      ...
      <section className="py-8 bg-white dark:bg-slate-900/30">
        <div className="mx-auto max-w-6xl px-6">
            ...
            {quickLinks.map((link) => {
              ...
              return (
                <Link key={link.title} href={link.url}>
                  <Card className="transition-all hover:shadow-lg border-slate-200 bg-white hover:border-blue-300 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-blue-700">
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="grid gap-8">
            {docSections.map((section) => {
              ...
              return (
                <Card key={section.title} className="border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
                  <CardHeader>
                    <div className="flex items-center gap-3">
                      <div className={`flex size-12 items-center justify-center rounded-xl ${section.colorClass}`}>
                  ...
                  <CardContent>
                    <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                      {section.links.map((link) => (
                        <Link key={link.title} href={link.url}>
                          <div className="group rounded-lg border border-slate-200 p-4 transition-all hover:border-slate-300 hover:bg-slate-50 hover:shadow-md dark:border-slate-800 dark:hover:border-slate-700 dark:hover:bg-slate-800/50">
                            <div className="flex items-start justify-between">
                              ...
                              <ArrowRight className="size-4 shrink-0 text-slate-400 transition-transform group-hover:translate-x-1 dark:text-slate-500" />
                            </div>
                          </div>
                        </Link>
                      ))}
      ...
      <section className="py-16 bg-slate-100 dark:bg-slate-900/50">
        <div className="mx-auto max-w-4xl px-6 text-center">
          ...
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            ...
            <Link href="/contact">
              <button className="rounded-lg bg-white px-6 py-3 font-semibold text-slate-900 hover:bg-slate-50 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700">
                Contact Support
              </button>
            </Link>
          </div>
